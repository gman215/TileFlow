...
- [ ] **T1 — Schemas.** Add `ROOM_FROM_IMAGE_JSON_SCHEMA` and the `RoomFromImageOut` Zod mirror to
...
- [ ] **T2 — Prompt.** Add `ROOM_FROM_IMAGE_V1` to `api/_lib/prompts.ts` covering every point listed
...
- [ ] **T4 — Image downscaling.** `client/src/utils/image.ts` exporting `downscaleToJpeg()` with the
...
- [ ] **T5 — Client API + DTO.** Add `roomFromImage()` to `client/src/api/ai.ts` with `AbortSignal`
      support and `AiError` code propagation.
      *Verify:* `cd client && npx tsc --noEmit` clean.

- [ ] **T6 — Store proposal state.** Add `shapeProposal`, `setShapeProposal`, `applyShapeProposal`
...
- [ ] **T7 — Ghost rendering.** Extend `client/src/components/Canvas/RoomShapeLayer.tsx` to draw
...
- [ ] **T8 — Upload UI.** `client/src/components/AI/PlanUpload.tsx`, rendered in `ShapePanel` above
...
- [ ] **T9 — Validation before offering** (`client/src/components/AI/PlanUpload.tsx`).
...
- [ ] **T10 — Disabled and failure states.** Wire `useAiAvailability()`; disable the control with a
